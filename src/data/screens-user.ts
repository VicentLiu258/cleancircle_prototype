  ...
  },
  {
    ...
    states: [
      { id: 'overview', label: '账户概览', blocks: [
        ...
        { kind: 'page-header', label: '用户详情 · U-08771', sub: '七分栏：账户概览 / 评测报告 / 周期与课表 / 运动与打卡 / 订阅与订单 / 能量值 / 消息与工单（§5.7）' },
        { kind: 'tabs', items: ['账户概览', '评测报告', '周期与课表', '运动打卡', '订阅订单', '能量值', '消息工单'], activeStep: 0 },
        { kind: 'form-row', label: '基础资料', sub: '昵称「小鹿」｜ 注册 2026-06-12 ｜ 来源：迁移（原小程序）' },
        { kind: 'form-row', label: '迁移信息', sub: '批次 #20260725-01 · 全部成功 · 迁移记录永久可查（移动端 B-12）', marker: 1 },
        { kind: 'form-row', label: '订阅状态', sub: '月卡 · 有效期至 2026-08-15 · 自动续费开（明细见 B21）' },
        { kind: 'form-row', label: '风险状态', sub: '无 ｜ 标签：大基数、减脂、新手' },
        { kind: 'alert', tone: 'info', label: '健康、周期与情绪答案默认折叠脱敏，特定角色二次确认后可查看（§5.7）', marker: 2 },
      ]},
      { id: 'report', label: '评测报告（脱敏锁定）', blocks: [
        sideUser,
        { kind: 'topbar', label: '用户中心 / 用户详情 / 评测报告', sub: '角色：健康运营' },
        { kind: 'page-header', label: '评测报告 · U-08771' },
        { kind: 'tabs', items: ['账户概览', '评测报告', '周期与课表', '运动打卡', '订阅订单', '能量值', '消息工单'], activeStep: 1 },
        { kind: 'form-row', label: '问卷版本与得分', sub: 'v3 · 2026-06-12 提交 ｜ 减脂 72 / 力量 45 / 柔韧 60（报告快照，§5.3）' },
        { kind: 'panel', label: '🔒 经期 / 情绪 / 身体答案（高敏感）', sub: '默认折叠脱敏；健康运营二次确认后查看，查看动作写审计日志（B27）', marker: 1 },
        { kind: 'button-secondary', label: '申请查看敏感答案（二次确认 + 审计）', marker: 2 },
      ]},
    ...
    annotations: {
      ...
      data: [
        '七分栏字段 — 用户表 / 问卷快照 / 课表 / 打卡 / 订阅 / 能量值流水 / 工单（§5.7）',
        '报告快照（问卷版本+答案+得分+话术） — §5.3 快照机制',
        '迁移记录 — 关联 B20 批次，永久可查（移动端 B-12）',
      ],
      actions: {
        primary: '按分栏查询用户信息',
      ...
      deps: ['移动端 S07 报告 / S09 今日 / S26 我的（用户侧同源数据）', '移动端 S16/S26 迁移记录', 'B21 订阅订单', 'B23 积分调整', 'B27 审计'],
      patches: ['H-09'],
    },
  },
  {
