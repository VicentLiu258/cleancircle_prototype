...
const mobileV1Screens: MobileScreenDef[] = [
  ...
  mobileScreen('S09', 'Today · 当天训练决策', 'A09', 'A', [
    { id: 'unchecked', label: '未 Check-in', blocks: [mStatus(), { kind: 'header', label: '早上好，{昵称}', sub: '今天 · Plan v3 · Check-in 待完成' }, { kind: 'card', label: '周期上下文：黄体期第 6 天', sub: '周期不是当天状态；仅作为推荐上下文' }, { kind: 'card', label: '今日状态：待确认', sub: '不会在你输入前预判疲劳', patch: true }, { kind: 'card', label: '今天的身体状态怎么样？', sub: '一个问题：感觉很好 / 能量低 / 我开始生理期了', to: 'S19', toState: 'daily-checkin', marker: 1 }, { kind: 'card', label: '原计划课程：全身活动 20min', sub: '完成 Check-in 后只调整今天', to: 'S10' }, mTab('今日')]},
    { id: 'checked', label: '已确认', blocks: [mStatus(), { kind: 'header', label: '今天是 Soft Day', sub: '用户已确认 · Check-in 已完成' }, { kind: 'card', label: '为什么：能量低 → 先降低冲击，保留今日训练意图', sub: 'decision_id dec_204 · rules_v1.0', marker: 1, patch: true }, { kind: 'card', label: '今日课程：低冲击核心活动 15min', sub: '结果：ADJUST · 原课程仍可撤销恢复', to: 'S10' }, { kind: 'button-secondary', label: '查看推荐原因' }, mTab('今日')]},
    { id: 'no-match', label: '无安全课程', blocks: [mStatus(), { kind: 'card', label: '今天没有符合你限制的安全课程', sub: '我们不会绕过安全过滤。可以休息，或回到训练档案补充资料。', marker: 1, patch: true }, { kind: 'button-primary', label: '今天休息' }, { kind: 'button-secondary', label: '查看训练档案', to: 'S26' }, mTab('今日')]},
  ], {
    goal: '让用户在 1 分钟内知道今天练什么、为什么，以及是否已确认当天状态。', entry: 'S08 / 日常打开 App', exit: ['S10', 'S19', 'S20', 'S21', 'S26'], role: baseMobileNotes.role,
    data: mobileData(['周期上下文 — cycle_facts/prediction', '今日 Check-in — daily_checkins', '今日 decision/candidate snapshot — 推荐服务', '课程客观属性 — approved Course Profile']),
    actions: { primary: '完成 Check-in 或开始课程', secondary: ['查看原因', '查看主备候选', '撤销当天调整', '更新周期事实'] }, statesDesc: ['未 Check-in', '已确认 Push/Soft/Warm', '调整后', '无安全课程', '加载/过期/离线'],
    triggers: ['未 Check-in 不预判身体状态', '周期事实不直接等于 Warm', 'Plan 不因当天 Check-in 改写未来'], deps: [...baseMobileNotes.deps], patches: ['V1-TODAY', 'D10'],
  }),
  ...
  mobileScreen('S19', '每日 / 首次 Check-in 与状态确认', 'A10', 'C', [
    { id: 'daily-checkin', label: '一问 Check-in', blocks: [mStatus(), { kind: 'card', label: '每日 Check-in · 今天一次', sub: '首 7 次显示简短解释；之后可跳过', marker: 1, patch: true }, { kind: 'header', label: '今天感觉怎么样？' }, { kind: 'button-secondary', label: 'Yes, I feel great · FEEL_GREAT', toState: 'recommendation' }, { kind: 'button-secondary', label: 'Energy is low · ENERGY_LOW', toState: 'recommendation' }, { kind: 'button-secondary', label: 'I got my period · PERIOD_STARTED', toState: 'period-confirm', marker: 2 }, { kind: 'button-secondary', label: '跳过今天', to: 'S09' }]},
    { id: 'recommendation', label: '系统建议与用户确认', blocks: [mStatus(), { kind: 'header', label: '系统建议：Soft Day', sub: '原因：能量低 → 先降低冲击并保留训练意图', marker: 1, patch: true }, { kind: 'card', label: '今天只会调整今天；未来 29 天不变', sub: '建议：ADJUST；备选：Backup A / B / C' }, { kind: 'button-primary', label: '确认 Soft Day', to: 'S09' }, { kind: 'button-secondary', label: '我想改成 Push / Warm', toState: 'state-selection' }]},
    { id: 'state-selection', label: '自主选择状态', blocks: [mStatus(), { kind: 'header', label: '选择今天的状态' }, { kind: 'button-secondary', label: 'Push Day · 保持原安排', to: 'S09' }, { kind: 'button-secondary', label: 'Soft Day · 适度训练', to: 'S09' }, { kind: 'button-secondary', label: 'Warm Day · 温柔照顾', to: 'S09' }]},
  ...
  mobileScreen('S25', '日历与训练回顾', 'A16', 'E', [{ id: 'default', label: '默认', blocks: [mStatus(), { kind: 'header', label: '日历 · 30 天滚动' }, { kind: 'calendar-grid', label: '✓ 完成 · ● 今日 · ↺ Re-plan · ◇ 自选 · □ 休息', sub: '过去日期锁定；颜色不是唯一状态信号', height: 210 }, { kind: 'card', label: '本月已完成 8 次 · 160 分钟', sub: '非惩罚式训练回顾，不显示中断归零的 streak', marker: 1, patch: true }, { kind: 'list-item', label: '今日 · 低冲击核心 15min [查看原因]', to: 'S10' }, { kind: 'list-item', label: '未来第 4 天 · Re-plan：Adjust', sub: '点击查看前后版本' }, mTab('日历')] }], {
    goal: '以 30 天日历展示计划、完成和变更，不制造连续打卡压力。', entry: 'Tab 日历 / S08 / S21', exit: ['S09', 'S10', 'S29'], role: baseMobileNotes.role,
    data: mobileData(['training_plan/days — 计划服务', '完成记录/反馈 — course_sessions', 'Re-plan diff — replan_runs']), actions: { primary: '查看某日课程', secondary: ['查看变更原因', '课程库浏览'] }, statesDesc: ['默认', '单日展开', 'Re-plan 标记', '无计划'], triggers: ['不支持补打历史日期；Daily Adapt 不改未来'], deps: [...baseMobileNotes.deps], patches: ['V1-CALENDAR'],
  }),
  mobileScreen('S26', '我的 · 训练档案与隐私', 'A21', 'E', [{ id: 'default', label: '默认', blocks: [mStatus(), { kind: 'header', label: '我的' }, { kind: 'card', label: '{头像} {昵称} · Profile v4', sub: '训练档案已更新于今天', marker: 1 }, { kind: 'header', label: '我的训练' }, { kind: 'list-item', label: '训练档案（目标/能力/限制/偏好/生命周期）', to: 'S28' }, { kind: 'list-item', label: '推荐记录与原因' }, { kind: 'list-item', label: '课程与训练历史' }, { kind: 'header', label: '隐私与设置' }, { kind: 'list-item', label: '周期事实与预测设置' }, { kind: 'list-item', label: '数据用途、导出与删除' }, mTab('我的')] }], {
    goal: '提供长期 Profile 的查看/修改和敏感数据控制入口。', entry: 'Tab 我的', exit: ['S28', 'S24'], role: baseMobileNotes.role,
    data: mobileData(['当前 User Training Profile 与字段来源', 'profile_consents、导出/删除请求', '推荐 decision 历史']), actions: { primary: '打开训练档案', secondary: ['查看推荐记录', '管理隐私和通知'] }, statesDesc: ['默认', '加载', '无 Profile', '无权限/删除处理中'], triggers: ['修改 Profile 先预览影响，再生成新版本并 Re-plan 未来'], deps: [...baseMobileNotes.deps, '隐私/账户服务'], patches: ['V1-PROFILE'],
  }),
  mobileScreen('S28', '训练档案修改与影响预览', 'A25', 'E', [{ id: 'edit', label: '修改档案', blocks: [mStatus(), { kind: 'header', label: '训练档案' }, { kind: 'input', label: '主目标', sub: 'FAT_LOSS · 可修改' }, { kind: 'input', label: '可训练时长', sub: '20 分钟' }, { kind: 'chip-row', label: '训练限制：膝盖（中度） · 跳跃耐受：低' }, { kind: 'card', label: '修改后会生成 Profile v5，并仅影响未来日期', sub: '已完成历史不覆盖；确认后启动 Re-plan', marker: 1, patch: true }, { kind: 'button-primary', label: '预览对未来计划的影响', toState: 'preview' }, { kind: 'button-secondary', label: '取消', to: 'S26' }]}, { id: 'preview', label: '影响预览', blocks: [mStatus(), { kind: 'header', label: '确认档案变化' }, { kind: 'card', label: '保持 18 天 · 调整 8 天 · 替换 4 天', sub: '变化来自新的能力/限制字段和规则版本' }, { kind: 'button-primary', label: '确认并开始 Re-plan', to: 'S21' }, { kind: 'button-secondary', label: '返回修改', toState: 'edit' }] }], {
    goal: '把档案修改从隐式重算改为可预览、可确认、可审计的版本变更。', entry: 'S26 训练档案', exit: ['S21', 'S26'], role: baseMobileNotes.role,
    data: mobileData(['Profile 字段与来源 — user_training_profiles', '变更 diff — change_previews', '未来计划影响 — replan_runs']), actions: { primary: '确认并 Re-plan', secondary: ['预览影响', '返回修改'], destructive: '删除敏感数据进入独立删除流程' }, statesDesc: ['编辑', '影响预览', '提交中', '失败可恢复'], triggers: ['只影响未来日期；不生成长报告'], deps: [...baseMobileNotes.deps], patches: ['V1-PROFILE-EDIT'],
  }),
...
const adminV1Screens: ScreenDef[] = [
  ...
  adminScreen('B10', 'User Profile 推导与安全校验', '§4 B10', 'B', [{ id: 'mapping', label: '字段推导', blocks: [
    ...aShell('标准与问卷 / Profile 推导', '角色：产品 / 安全审核'), { kind: 'page-header', label: 'User Training Profile 推导 · onboarding_v1.1', sub: '替代旧版加权分与长报告：结构化字段、来源、冲突与安全结果', patch: true }, { kind: 'table', cols: ['目标字段', '来源问题', '推导结果', '安全校验', '状态'], items: ['primary_goal ｜ Q03 ｜ FAT_LOSS ｜ — ｜ ✓', 'fitness_capacity ｜ Q06 ｜ L2 ｜ 值域 L1–L5 ｜ ✓', 'limitations.knee ｜ Q08 ｜ MODERATE ｜ Hard Filter 输入 ｜ ✓', 'lifecycle.stage ｜ Q15 ｜ PREGNANT ｜ V1 Block ｜ ⚠', 'equipment ｜ Q12 ｜ 不生成 ｜ 已从用户档案移除 ｜ ✓'], marker: 1 }, { kind: 'split', label: '回答快照', sub: 'Profile JSON + 字段来源', items: ['问卷提交 sub_1042', 'Profile v4', '字段来源可点击回到题目'], right: ['冲突：主/次目标均未选择', '冲突：盆底字段缺少课程标签', '动作：阻止发布/要求补字段'], marker: 2 }, { kind: 'button-primary', label: '保存推导规则并提交审核' }, { kind: 'button-secondary', label: '运行典型夹具回归', to: 'B13' }] }], {
