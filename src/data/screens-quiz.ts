...
export const screensQuiz: ScreenDef[] = [
  {
    ...
    annotations: {
      ...
      deps: ['移动端 S04 问卷逐题页（渲染版本来源）', '移动端 S07 报告（问卷版本快照）', '审批流 §6 问卷与报告话术'],
      patches: [],
    },
  },
  {
  ...
  },
  {
    ...
    annotations: {
      ...
      triggers: [
        '审核通过 → 发布新版本；移动端 S07 新生成报告使用新版本快照',
        '审核驳回 → 回退草稿态并回显驳回原因',
      ],
      deps: ['移动端 S06 报告生成 / S07 报告页（话术渲染）', '移动端 S28 周期资料修改（触发重测）', '审批流 §6 问卷与报告话术'],
      patches: ['H-04', 'H-06'],
    },
  },
];
