...
// 后台 IA：对齐 cleancircle_web MENU_CONFIG（2026-08-11）
const IA: { menu: string; prio: string; pages: { label: string; sid?: string }[] }[] = [
  ...
  { menu: '消息与触达', prio: 'P0', pages: [
    { label: '模板 B15', sid: 'B15' }, { label: '触发器 B16', sid: 'B16' },
    { label: '触达任务 B28', sid: 'B28' }, { label: '发送记录 B54', sid: 'B54' },
  ]},
  ...
  { menu: '会员与财务', prio: 'P0', pages: [
    { label: '套餐 B22', sid: 'B22' }, { label: '订阅看板 B53', sid: 'B53' },
    { label: '订阅订单 B21', sid: 'B21' }, { label: '退款 B24', sid: 'B24' }, { label: '对账 B25', sid: 'B25' },
  ]},
  ...
  { menu: '系统管理', prio: 'P0', pages: [
    { label: '登录 B01', sid: 'B01' }, { label: '角色权限 B26', sid: 'B26' },
    { label: '管理员 B26', sid: 'B26' }, { label: '审计 B27', sid: 'B27' },
  ]},
];
...
const ROLES: { role: string; scope: string; limit: string }[] = [
  ...
  { role: '内容运营', scope: '视频、课程、标签、知识库、挑战赛、投放', limit: '无用户健康明细和财务导出权限' },
  { role: '课程/健康运营', scope: '问卷、话术、周期建议、排课规则、知识库健康类终审', limit: '规则发布需二次审核（H-04）' },
  { role: '用户运营/CRM', scope: '用户查询、标签分群、消息触达、发送记录、迁移结果', limit: '健康信息脱敏；不可修改支付流水' },
  { role: '客服', scope: '用户档案、订阅、迁移、企微入口', limit: '手机号脱敏；能量值补发需审批' },
  { role: '财务', scope: '套餐、订阅看板、订单、订阅、退款、对账与导出', limit: '无问卷答案和内容编辑权限' },
    ...
    <div className="mx-auto max-w-5xl px-6 py-8">
      ...
      <div className="mt-3 rounded-lg border border-gray-200 bg-white p-5">
        ...
        <p className="mt-2 text-sm text-gray-600">
          ...
          信息架构已对齐 <span className="font-mono text-[12px]">cleancircle_web</span> 菜单。
        </p>
