...
// 把含 Bxx 的文本渲染成可点击引用；Sxx（移动端屏幕）仅高亮不可点
  ...
  return (
    <>
      {parts.map((p, i) => {
        ...
        if (/^S\d{2}$/.test(p)) {
          return (
            <span key={i} className="font-mono text-gray-500" title="移动端线框工程（cleancircle-wireframe）中的屏幕编号">
              {p}
            </span>
          );
        }
  ...
  return (
    <div className="flex h-full min-h-0">
      ...
      <main className="flex min-w-0 flex-1 flex-col items-center overflow-y-auto bg-gray-100 py-5">
        ...
        <p className="mt-3 text-[11px] text-gray-400">带 → 的按钮/表格行可点击跳转 · 左侧菜单高亮项可切换页面 · 灰色圆点数字对应右栏标注 · amber 虚线 = 产品补全</p>
      </main>
      ...
      <aside className="w-80 shrink-0 overflow-y-auto border-l border-gray-200 bg-white">
        ...
        <Field label="依赖（移动端屏幕 / 后台模块）">
          <ul className="list-disc space-y-0.5 pl-4">
            {a.deps.map((d, i) => <li key={i}><ScreenRefs text={d} onNavigate={selectScreen} /></li>)}
          </ul>
        </Field>
